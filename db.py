...
async def get_all_settings() -> dict:
    """Return settings: env vars are source of truth, Supabase is fallback."""
    KNOWN_KEYS = [
    ...
    out: dict = {}
    # First priority: environment variables (source of truth)
    for k in KNOWN_KEYS:
        env_val = os.getenv(k, "")
    ...
    # Second priority: Supabase for values not set via env
    try:
        db = await _adb()
        ...
        for row in (result.data or []):
            ...
            if k == "TEST_KEY":
                continue
            # Only show Supabase value if env var is NOT set
...
async def get_setting(key: str, default: str = "") -> str:
    """Get setting: env var FIRST (single source of truth), then Supabase as fallback."""
...
async def get_enabled_tools() -> list:
    """Get enabled tools: env var first, then Supabase."""
