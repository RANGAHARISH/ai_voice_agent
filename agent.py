...
def load_db_settings_to_env() -> None:
    """Load Supabase settings into os.environ ONLY for keys NOT already set by env vars.
    VPS environment variables always take priority (single source of truth)."""
    ...
    try:
        ...
        for row in (result.data or []):
            k = row.get("key", "")
            v = row.get("value", "")
            # NEVER override an env var that's already set
